...
#Load values from the dwg.cfg file
# CONFIG
...
# DROP TABLES - Drop tables so we can reset our database and test the ETL pipeline
...
# CREATE TABLES - SQL Queries to create 7 tables (staging_events, staging_songs
# songplays, users, songs, artists and time)
...
CREATE TABLE IF NOT EXISTS songplays (
  songplay_id   VARCHAR     PRIMARY KEY NOT NULL SORTKEY,
...
# STAGING TABLES - Populating the staging tables from the json data
...
# FINAL TABLES - Populating the fact table (songplays) and the dimension tables 
...
songplay_table_insert = ("""
    INSERT INTO songplays (   songplay_id,
                              ...
                              location,
                              user_agent)
    SELECT  e.itemInSession  AS songplay_id,
            e.ts             AS start_time,
            e.userId         AS user_id,
            e.level          AS level,
...
# QUERY LISTS - Create list of all the queries for each sub divisions
